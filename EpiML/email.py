...
@celery.task()
def send_submit_job_email(recipients,jobid, security_code):
    subject = 'Notice from ShiLab'
    sender = app.config['MAIL_USERNAME']
    ...
    msg.body = 'Dear {},\nThank you for using EpiML web server.\n\nYour job has been submitted. ' \
               'The processing time depends on the scale of your data. You can check job status and retrieve result ' \
               'via the following link\n Result Link: http://127.0.0.1:5000/processing/{}_{}\n\n' \
               'Sincerely,\n' \
               'ShiLab'.format(recipients[0],jobid, security_code)
